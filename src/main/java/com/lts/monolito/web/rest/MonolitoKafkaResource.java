...
@RestController
@RequestMapping("/api/monolito-kafka")
public class MonolitoKafkaResource {

    private final Logger log = LoggerFactory.getLogger(MonolitoKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public MonolitoKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
