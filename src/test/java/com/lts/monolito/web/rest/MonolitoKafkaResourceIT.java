...
import com.lts.monolito.config.EmbeddedKafka;
import com.lts.monolito.config.KafkaSseConsumer;
import com.lts.monolito.config.KafkaSseProducer;
...
class MonolitoKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/monolito-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/monolito-kafka/consume")
